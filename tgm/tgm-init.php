 ...
 */
require_once NOKRI_API_PLUGIN_FRAMEWORK_PATH . '/tgm/class-tgm-plugin-activation.php';
...
function sbAPI_themes_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'               => esc_html__( 'SB Framework', 'nokri-rest-api' ), // The plugin name.
			'slug'               => 'sb_framework', // The plugin slug (typically the folder name).
			'source'             => NOKRI_API_PLUGIN_URL.'tgm/sb_framework.zip', // The plugin source.
		...
		array(
			'name'               => esc_html__( 'Woocommerce', 'nokri-rest-api' ), // The plugin name.
			...
			'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
			'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.   
			'external_url' => esc_url('https://downloads.wordpress.org/plugin/woocommerce.zip'), // If set, overrides default API URL and points to an external URL.
			'is_callable'        => '', // If set, this callable will be be checked for availability to determine if a plugin is active.
		),
